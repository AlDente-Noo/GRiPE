...
public class DNA  implements Serializable{
	...
	private void setOccupancyAndClosenessVectorsFree(Cell n){
		...
		openDNA(n, Constants.FIRST, strand.length);
	}
	...
	private void recomputeTFAffinityLandscapeOnBinding(Cell n, int position, int size){
		recomputeTFAffinityLandscapeOnRepression(n, position, position+size-1);
	}

	/**
	 * 	recomputes the TF affinity landscape when a molecule represses the DNA region
	 */
	private void recomputeTFAffinityLandscapeOnRepression(Cell n, int boundaryLeft, int boundaryRight){
...
//		}
	...
	public void repressDNA(Cell n, int boundaryLeft, int boundaryRight) {
	...
	public void repress(Cell n, int boundaryLeft, int boundaryRight) {
//		boundaryLeft = updateLeftBoundary(boundaryLeft);
//		boundaryRight = updateRightBoundary(boundaryRight);
//		for (int bpIdx = boundaryLeft; bpIdx <= boundaryRight; bpIdx++) {
//			if (this.closed[bpIdx] == Constants.BP_IS_OPEN) {
...
//			}
//		}
		repressDNA(n, boundaryLeft, boundaryRight);
		this.recomputeTFAffinityLandscapeOnRepression(n, boundaryLeft, boundaryRight);
	}
	 ...
	 */
	public void openDNA(Cell n, int position, int size){
		int end = Math.min(position+size, strand.length);
		int start = Math.max(0, position);
