...
public class TF extends DBP  implements Serializable {
	...
	public void act(Cell n, ProteinEvent pe){
		...
		if(this.position==Constants.NONE){
			...
			assert pe.nextAction == Constants.EVENT_TF_BINDING;
			...
			if(pe.nextAction == Constants.EVENT_TF_RANDOM_WALK_JUMP){
				...
				if (this.isRepressingDNA() || this.isRepressed(n)) {
					if (n.isInDebugMode()) {
						n.printDebugInfo(pe.time + ": attempted to hop TF " + this.ID + " of type " + n.TFspecies[speciesID].name + " from position " + position + " to " + pe.position + ", but the TF is repressing or repressed");
					}
				} else {
					...
					else if (pe.position == this.position) { //the TF hops and rebinds at the same position
						assert pe.isHoppingEvent; // FG: it seems that if the event is hop then isHoppingEvent is always true
						hopMoleculeSamePos(n, pe.time, this.position);
					}
					else { //the TF hops and rebinds at a different position
				...
				if (this.isRepressingDNA() || this.isRepressed(n)) {
					if (n.isInDebugMode()) {
						n.printDebugInfo(pe.time + ": attempted to slide left TF " + this.ID + " of type " + n.TFspecies[speciesID].name + " from position " + position + " to " + pe.position + ", but the TF is repressing or repressed");
					}
				} else {
					if (n.dna.isAbsorbing && pe.position < 0) {
						...
						if (pe.position >= 0) {
							...
							if (n.isInDebugMode()) {
								n.printDebugInfo(pe.time + ": attempted to slide left TF "+this.ID+" of type "+n.TFspecies[speciesID].name+" from position "+position+", but the left border is reached");
							}
				...
				if (this.isRepressingDNA() || this.isRepressed(n)) {
					if (n.isInDebugMode()) {
						n.printDebugInfo(pe.time + ": attempted to slide right TF " + this.ID + " of type " + n.TFspecies[speciesID].name + " from position " + position + " to " + pe.position + ", but the TF is repressing or repressed");
					}
				} else {
					if (n.dna.isAbsorbing && pe.position >= (n.dna.strand.length - this.size)) {
						...
						if (pe.position < (n.dna.strand.length - this.size)) {
							...
							if (n.isInDebugMode()) {
								n.printDebugInfo(pe.time + ": attempted to slide right TF "+this.ID+" of type "+n.TFspecies[speciesID].name+" from position "+position+", but the right border is reached");
							}
							resetPosition(pe.time);
						}
						if (pe.isHoppingEvent) { // FG: donno what is this for
							this.changeDirection(n, pe.time);
						}
					}
				}
...
//		}
	...
	public boolean unbindMolecule(Cell n, double time){
		...
		if(n.isInDebugMode()){
			if(!unbound){
				...
				n.printDebugInfo(time+": TF "+this.ID+" of type "+n.TFspecies[speciesID].name+" unbound from the DNA at position "+ this.position);
			}
		}
	...
	public void hopMoleculeSamePos(Cell n, double time, int position) {
		...
		if (n.isInDebugMode()) {
			n.printDebugInfo(time + ": TF " + this.ID + " of type " + n.TFspecies[speciesID].name + " hop at the same position " + position);
		}
	}
	...
	public int hopMolecule(Cell n, double time, int newPosition){
		...
		if(n.isInDebugMode()){
			if(bound==ID){
				...
				String str = time + ": failed to hop TF " + this.ID + " of type " + n.TFspecies[speciesID].name + " to position " + newPosition + " which is occupied or repressed, ";
